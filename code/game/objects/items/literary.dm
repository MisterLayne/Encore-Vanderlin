...
/obj/item/textbook/attacked_by(obj/item/I, mob/living/user)
	if(istype(I, /obj/item/natural/feather))
		if(skilltoteach)
			return
		var/skill = input(user, "What kind of textbook will you write", "AKAN") as null|anything in teachable_skills
		if(!skill)
			return
		...
		var/level_name = input(user, "What level of difficulty should the textbook be?", "AKAN") as null|anything in possible_skill_levels
		if(!level_name)
			return
		if(skilltoteach)
			return
