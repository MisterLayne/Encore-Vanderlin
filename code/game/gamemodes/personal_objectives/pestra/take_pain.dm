/datum/objective/personal/take_pain
	...
	rewards = list("3 Triumphs", "Erdl grows stronger", "Erdl blesses you (+1 Constitution)")
...
/datum/objective/personal/take_pain/proc/on_pain_transferred(datum/source, amount)
	...
	if(progress_ratio < 0.25)
		to_chat(owner.current, span_green("You feel a small amount of pain flow through you. Erdl is pleased, but there is much more suffering to relieve."))
	else if(progress_ratio < 0.5)
		to_chat(owner.current, span_green("The pain you've taken weighs heavily upon you. Keep going, Erdl's work is not yet done."))
	else if(progress_ratio < 0.75)
		to_chat(owner.current, span_green("The agony you've absorbed is substantial. You're making good progress in Erdl's name."))
	else if(progress_ratio < 1)
		to_chat(owner.current, span_green("The pain is nearly overwhelming, but you can sense you're close to completing Erdl's task."))
