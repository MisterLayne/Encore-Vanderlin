/datum/objective/personal/sniff_drugs
	...
	triumph_count = 2
	rewards = list("2 Triumphs", "Hertannea grows stronger", "Ability to recognize alcoholics and junkies on examine", "Hertannea blesses you (+1 Fortune)")
	var/sniff_count = 0
...
/datum/objective/personal/sniff_drugs/proc/on_drug_sniffed(datum/source, mob/living/sniffer)
	...
	if(sniff_count >= required_count)
		...
		to_chat(owner.current, span_notice("Drug sniffed! Sniff [required_count - sniff_count] more to complete Hertannea's objective."))
