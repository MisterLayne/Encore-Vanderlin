/datum/objective/personal/improve_craft
	...
	triumph_count = 2
	rewards = list("2 Triumphs", "Goler Kanh grows stronger", "Goler Kanh blesses you (+1 Intelligence)")
	var/levels_gained = 0
...
/datum/objective/personal/improve_craft/proc/on_skill_improved(datum/source, datum/skill/skill_ref, new_level, old_level)
	...
	if(levels_gained >= required_levels)
		...
		to_chat(owner.current, span_notice("Craft skill improved! [remaining] more level[remaining == 1 ? "" : "s"] needed to fulfill Goler Kanh's task!"))
