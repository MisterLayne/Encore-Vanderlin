/datum/action/cooldown/spell/persistence
	...
	invocation = "Al'Aqshir deems your persistence worthy!"
...
/datum/action/cooldown/spell/persistence/cast(mob/living/cast_on)
	...
	if(cast_on.mob_biotypes & MOB_UNDEAD)
		...
		if(ishuman(cast_on)) //BLEED AND PAIN
			...
			human_target.visible_message(span_danger("[cast_on]'s wounds become inflamed as their vitality is sapped away!"), span_userdanger("Al'Aqshir inflames my wounds and weakens my body!"))
		return
	...
	if(situational_bonus > 0.25)
		to_chat(owner, "Channeling Al'Aqshir's power is easier in these conditions!")
