...
GLOBAL_LIST_EMPTY(archdevils_cullings)

/datum/round_event_control/archdevils_culling
	name = "Hell's Culling"
	track = EVENT_TRACK_INTERVENTION
	typepath = /datum/round_event/archdevils_culling
	weight = 8
...
/datum/round_event_control/archdevils_culling/canSpawnEvent(players_amt, gamemode, fake_check)
	. = ..()
	if(!.)
		return FALSE
	if(GLOB.patron_follower_counts[/datum/patron/inhumen/archdevils::name] < 2)
		return FALSE

/datum/round_event/archdevils_culling/start()
	...
	var/grand_culling = is_ascendant(ARCHDEVILS) || prob(33)
	...
	for(var/i in 1 to max_pairs)
		...
		GLOB.archdevils_cullings += new_duel

		// Notify first chosen
		bordered_message(first_chosen, list(
			span_userdanger("YOU ARE THE ARCHDEVILS'S CONTESTANT!"),
			span_red("Weak should feed the strong, that is Hell's will. Prove that you are not weak by eating the heart of [span_notice(second_chosen.real_name)], the [second_chosen.job] and gain unimaginable power in turn. Fail, and you will be the one eaten."),
		))
		if(grand_culling)
		...
		bordered_message(second_chosen, list(
			span_userdanger("YOU ARE THE ARCHDEVILS'S CONTESTANT!"),
			span_red("Weak should feed the strong, that is Hell's will. Prove that you are not weak by eating the heart of [span_notice(first_chosen.real_name)], the [first_chosen.job] and gain unimaginable power in turn. Fail, and you will be the one eaten."),
		))
		if(grand_culling)
...
/datum/culling_duel/proc/partial_victory(mob/living/winner, mob/living/loser)
	if(winner)
		winner.add_stress(/datum/stress_event/archdevils_culling_finished)
		winner.adjust_triumphs(1)
		adjust_storyteller_influence(ARCHDEVILS, 20)
		to_chat(winner, span_notice("Your rival's heart has been DESTROYED! While not the glorious consumption Archdevils has desired, you have overcome the culling nevertheless."))

	finish_culling(winner, loser)

/datum/culling_duel/proc/finish_culling(mob/living/winner, mob/living/loser)
	if(winner)
		winner.remove_stress(/datum/stress_event/archdevils_culling_unfinished)
		winner.remove_spell(/datum/action/cooldown/spell/undirected/seek_rival)

	if(loser)
		loser.remove_stress(/datum/stress_event/archdevils_culling_unfinished)
		loser.remove_spell(/datum/action/cooldown/spell/undirected/seek_rival)
		to_chat(loser, span_boldred("You have FAILED Archdevils for the LAST TIME!"))
		loser.gib()
