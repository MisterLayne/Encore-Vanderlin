...
/datum/triumph_buy/communal/angros_retirement_fund/on_activate()
	...
	if(length(eligible) == 1)
		...
		adjust_triumphs(C, total_pool, FALSE, "Angros' Retirement Fund", override_bonus = TRUE)

		bordered_message(world, list(
			span_reallybig("A total of [total_pool] triumph[total_pool == 1 ? " has" : "s have"] been redistributed to 1 beneficiary from the Angros' Retirement Fund!"),
		))
	...
	for(var/client/C in distribution)
		if(distribution[C] > 0)
			...
			adjust_triumphs(C, distribution[C], FALSE, "Angros' Retirement Fund", override_bonus = TRUE)

	bordered_message(world, list(
		span_reallybig("A total of [total_pool] triumph[total_pool == 1 ? " has" : "s have"] been redistributed between [number_of_beneficiaries] beneficiaries from the Angros' Retirement Fund!"),
	))
