...
/datum/devotion_task/golerkanh_craft
	...
	desc = "Craft items to honor Goler Kanh"
	devotion_reward = 3
	progression_reward = 2
	cooldown_time = 15 SECONDS
	signal_type = COMSIG_ITEM_CRAFTED

/datum/devotion_task/golerkanh_craft/on_complete(mob/living/carbon/human/user, list/signal_args)
	. = ..()
	if(.)
		user.visible_message(
			span_notice("[user] completes their work with devoted precision."),
			span_notice("My labor honors Goler Kanh.")
		)
