...
/datum/devotion/proc/excommunicate()
	if(!HAS_TRAIT(holder_mob, TRAIT_FANATICAL))
		...
		devotion = -1
		to_chat(holder_mob, span_userdanger("I have been excommunicated! The Aspects no longer listen to my prayers nor my requests."))
		STOP_PROCESSING(SSprocessing, src)
...
/datum/devotion/proc/recommunicate()
	if(!HAS_TRAIT(holder_mob, TRAIT_FANATICAL))
		...
		devotion = 0
		to_chat(holder_mob, span_boldnotice("I have been welcomed back into the folds of the divine."))
		if(passive_devotion_gain || passive_progression_gain)
			START_PROCESSING(SSprocessing, src)
